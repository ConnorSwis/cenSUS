...
function fuzzySearch(searchTerm, items, threshold = 75) {
    const searchKeys = items.map((item) => item.search_key);
    // Perform fuzzy search
    const matches = fuzzysort.go(searchTerm, searchKeys, { limit: false });
    ...
    return filteredSortedMatches.map((match) => ({
        search_key: match.target,
        score: -match.score, // Adjusting score to match the Python example's positive score logic
    }));
}

function findClosestMatches(searchQuery, threshold = 75) {
    // Prepare items
    const stateItems = Object.entries(FIPS).map(([state, counties]) => {
        const fipsCode =
            typeof counties === "string"
                ? counties
                : Object.values(counties)[0].substring(0, 2);
        return {
            search_key: state.toLowerCase(),
            state,
            county: null,
            fips: fipsCode,
        };
    ...
    const countyItems = Object.entries(FIPS)
        .filter(([, counties]) => typeof counties === "object")
        .flatMap(([state, counties]) =>
            Object.entries(counties).map(([county, fips]) => ({
                search_key: `${state} ${county}`.toLowerCase(),
                state,
                county,
                fips,
            }))
    ...
    const combinedItems = [...stateItems, ...countyItems];
    ...
    return matches.map((match) => {
        const item = combinedItems.find(
            (item) => item.search_key === match.search_key
        );
        return { fips: item.fips, state: item.state, county: item.county };
    });
}

function render(searchQuery) {
    const matches = findClosestMatches(searchQuery);
    const resultsDiv = document.getElementById("results");
    resultsDiv.innerHTML = matches
        .map(
            (match, index) =>
                `<div>
                <input type="radio" id="match${index}" name="searchResult" value="${match.fips
                }">
                <label for="match${index}">${match.state} ${match.county ? `(${match.county})` : ""
                }</label>
            </div>`
        )
        .join("");
...
render(document.getElementById("searchQuery").value);
...
document
    .getElementById("variableForm")
    .addEventListener("submit", (event) => {
        event.preventDefault();
        const selectedFIPS = document.querySelector(
            'input[name="searchResult"]:checked'
        )?.value;
        const selectedVariables = Array.from(
            ...
            .join(",");
        const requestURL = `http://localhost:5000/census?variables=${selectedVariables}&fips=${selectedFIPS}`;
        const xhr = createCORSRequest("GET", requestURL);
        if (!xhr) {
...
const variableMapping = {
    ...
    individuals_below_poverty: "B17001_002E",
    labor_force: "B23025_002E",
    employed: "B23025_004E",
    unemployed: "B23025_005E",
    less_than_high_school_graduate: "B15003_002E",
    high_school_graduate: "B15003_017E",
    some_college_or_associates_degree: "B15003_018E",
    ...
    housing_units_with_a_mortgage: "B25027_002E",
    median_monthly_housing_costs_with_a_mortgage: "B25105_001E",
    housing_units_without_a_mortgage: "B25027_010E",
    median_monthly_housing_costs_without_a_mortgage: "B25106_001E",
};
...
Object.keys(variableMapping).forEach((key) => {
    ...
    checkbox.id = key;
    checkbox.name = key;
    checkbox.value = variableMapping[key];
